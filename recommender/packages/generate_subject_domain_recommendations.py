...
def generate_subject_domain_recommendations(student: Student, SUBJECT_DOMAIN_KEYWORDS_DATA_PATH, SUBJECT_DOMAIN_MODEL_PATH, similarity_type='cosine', k=5, threshold=0.6):
    ...
    df_subject_domain_similarities, actual_string = generate_all_subject_domain_similarities(student, subject_domains, SUBJECT_DOMAIN_MODEL_PATH, similarity_type, threshold)

    df_recommendations = df_subject_domain_similarities.head(k)

    return df_recommendations['Subject Domain'].values.tolist(), actual_string
